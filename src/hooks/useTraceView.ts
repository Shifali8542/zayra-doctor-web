...
export const useTraceView = (caseId?: number) => {
  ...
  // Step 3: Fetch waveform — fetches ALL channels, downsample=8
  ...
  // Available leads from current waveform record
  const availableLeads = useMemo(() => {
    if (!waveformData?.channel_names) return ['II'];
    return waveformData.channel_names;
  }, [waveformData]);

  // Extract selected lead signal — fallback chain: selected → II → ii → first
